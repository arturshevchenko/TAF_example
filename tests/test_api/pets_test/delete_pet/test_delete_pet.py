...
def test_delete_not_found_id_pet():
    # send request
    response = PetsService() \
        .delete_pet(pet_id=DataGenerator.random_uuid4())
    ...
    response.should_have(body("$..message", hamcrest.contains_string("java.lang.NumberFormatException")))
...
def test_delete_invalid_id_pet():
    # send request
    response = PetsService() \
        .delete_pet(pet_id=DataGenerator.string_of(5))
