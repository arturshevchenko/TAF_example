...
def test_get_not_found_id_pet():
    # send request
    response = PetsService() \
        .get_pet_id(pet_id=DataGenerator.random_uuid4())
    ...
    response.should_have(body("$..message", contains_string("java.lang.NumberFormatException")))
...
def test_get_invalid_id_pet():
    # send request
    response = PetsService() \
        .get_pet_id(pet_id=DataGenerator.string_of(5))
