...
class PetsService(BaseService):
    ...
    def create_pet(self, pet: PetModel) -> AssertableResponse:
        (
            self._request.set_path(Endpoint.PETS.value)
    ...
    def get_pet_id(self, pet_id) -> AssertableResponse:
        pet_id_url = Endpoint.PET_ID.value.format(pet_id=pet_id)
        self._request.set_path(pet_id_url).set_method(ApiMethod.GET)
        return AssertableResponse(self._request.send())
    ...
    def delete_pet(self, pet_id) -> AssertableResponse:
        pet_id_url = Endpoint.PET_ID.value.format(pet_id=pet_id)
        self._request.set_path(pet_id_url).set_method(ApiMethod.DELETE)
        return AssertableResponse(self._request.send())

    @allure.step("get pets")
    @service_logger
    def get_pets(self) -> AssertableResponse:
        self._request.set_path(Endpoint.PETS.value).set_method(ApiMethod.GET)
        return AssertableResponse(self._request.send())
    ...
    def find_by_status(self, status=None) -> AssertableResponse:
        (
            self._request.set_path(Endpoint.PET_FIND_BY_STATUS.value)
