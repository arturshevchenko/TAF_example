...
class AssertableResponse(object):
                ...
                if response.request.body:
                    try:
                        request_body = json.dumps(
                            json.loads(response.request.body), indent=6
                        )
    ...
    def status_code(self, code):
        if AppConfigs.LOGS:
            logging.info(
                f"{Colors.Magenta}ASSERT: Status code should be {code}{Colors.Reset}"
            )
    ...
    def should_have(self, *condition):
        for condition_arg in condition:
            if AppConfigs.LOGS:
                logging.info(
                    f"{Colors.Magenta}Check: {str(condition_arg)}{Colors.Reset}"
                )
    ...
    def should_not_have(self, condition):
        if AppConfigs.LOGS:
            logging.info(
                f"{Colors.Magenta}Check not contains: {str(condition)}{Colors.Reset}"
            )
