...
class DataGenerator:
    ...
    def email():
        return "ttmi.test.driver+{0}@gmail.com".format(DataGenerator.string_numbers_of(15))
    ...
    class Thai:
        ...
        def string_with_spec_chars_of(cls, length):
            ...
            key = ''
            for _ in range(length):
                key += random.choice(''.join(cls.thai_characters) + string.digits + string.punctuation)
            return str(key)
        ...
        def string_alphanumeric_dash_dot_huphen(cls, length):
            key = ''
            for _ in range(length):
                key += random.choice(''.join(cls.thai_characters) + string.digits + '-_.')
            return str(key)
